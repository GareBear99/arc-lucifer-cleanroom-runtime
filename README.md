...
This release adds a structured goal compiler, shadow predicted-vs-actual comparison, and a promotion-court review step for self-improvement runs.
...
## FixNet archive embedding

This runtime now supports a first-class FixNet dossier lane. FixNet cases can be recorded as structured evidence packets and embedded into the archive-visible branch state so self-improvement history and fix intelligence remain linked instead of splitting across temp files and memory-only traces.


## FixNet repair intelligence (DARPA-grade)

FixNet is the repair-intelligence sidecar to the deterministic runtime spine.
It separates:
- **FixLedger** (stored fix objects + semantic fix-to-fix lineage)
- **FixConsensus** (trust/analytics, quarantines, success rates)
- **FixNoveltyFilter** (duplicate/variant detection before polluting the store)
- **EmbeddedFixArchive** (archive-visible mirrors of selected fixes)

The runtime auto-emits FixNet records for self-improve validation and promotion failures so repair knowledge compounds instead of disappearing into logs.
